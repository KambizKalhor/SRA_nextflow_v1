...
params.input_dir = "/scratch1/kkalhor/66_metagenomes_main/inputs/"
...
params.megahit_results = "$projectDir/results/05_megahit_results"
...
include { MEGAHIT } from './modules/05_megahit.nf'

include { METAQUAST } from './modules/06_metaquast.nf'

workflow {
    // Create a channel from input files
    input_files = Channel.fromFilePairs("${params.input_dir}/*_{1,2}.fastq", checkIfExists: true)
    ...
    // megahit we use megahit instead of metaspades because it needs less ram
    MEGAHIT(TRIMMOMATIC.out.process_groupA_TRIMMOMATIC_output)
