...
params.multiqc_reports_before_trimmomatic = "$projectDir/results/02_first_multiqc_report"
params.trimmomatic_results = "$projectDir/results/03_trimmomatic_results"
params.second_quality_control_results = "$projectDir/results/04_second_quality_control_results"
params.multiqc_reports_after_trimmomatic = "$projectDir/results/05_second_multiqc_report"
params.multiqc_reports_alltogether = "$projectDir/results/06_total_multiqc_report"
params.metaspades_results = "$projectDir/results/07_metaspade_results"


params.metaquast_results = "$projectDir/results/08_metaquast_results"
...
include { MULTIQC } from './modules/04_multiqc.nf'
...
workflow.onComplete {
    ...
    //if (workflow.success) {
    //    log.info "Running final scripts including multiqc..."
    //    "bash multiqc_script.sh".execute().waitFor()}
}
